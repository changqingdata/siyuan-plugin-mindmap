...
| 快捷键 | `Ctrl + 空格` 切换导图 / 大纲，`Alt + 空格` 并排打开 | 可在思源「设置 → 快捷键」里改键 |

> 快捷键在 macOS 上写作 `⌘ + 空格` / `⌥ + 空格`。
> ⚠️ 思源内部把「空格」存成一个**字面空格字符**（不是 `Space` 这个词），
> 所以插件里的绑定串就是 `⌘ `（`⌘` 后跟一个空格）——
> 这也是为什么「查看快捷键」里会把空格渲染成可见的「空格」二字：
...
那是思源与其它插件共用的命名空间，导图必须整族让开。
（插件自己的两条全局快捷键是 `⌘空格` / `⌥空格`，**不属于**这一族 ——
...
- **「查看快捷键」** —— 弹出一张**分组表格对话框**（「键位 | 作用」两列，12 组 37 行），
  而不是一条十几秒就消失的通知。按平台换算：Windows 显示 `Ctrl + 空格`、
  macOS 显示 `⌘ + 空格`；没有键位的行（点复选框、右键菜单这类）显式写「菜单操作」

...
2. 选择 **插件 → 大纲导图**，然后在三个模式里挑一个：

   | 模式 | 作用 |
   | --- | --- |
   | **大纲模式**（关闭导图） | 就是这个列表块本来的样子，什么都不加 |
   | **导图模式** | 列表块变成导图；默认用**逻辑结构图** |
   | **并排模式**（大纲 + 导图） | 左边保留大纲、右边开一张跟随的导图，改哪边另一边都跟着变 |

3. 想换布局（逻辑结构图 / 思维导图 / 树状图）**在导图自己的工具条上切**，
   不用回块菜单 —— 那里是「导图长什么样」该待的地方，点完立刻看得到效果

> 菜单里只有「要不要导图、要不要并排」，**布局不在菜单里**。
> 这是有意的：老菜单把「大纲视图 / 逻辑结构图 / 思维导图 / 树状图 / 并排查看」
> 五项平铺，混了两个层级的东西（「要不要导图」和「导图长什么样」）。
>
> 顺带一条容易踩的：**已经在导图模式时再点「导图模式」，不会把布局重置回默认** ——
> 否则你手动切到「树状图」之后点一下菜单就被打回逻辑结构图了。
...
| 快捷键 | `Ctrl + Shift + D` 切换导图 / 大纲，`Ctrl + Shift + X` 并排打开 | 可在思源「设置 → 快捷键」里改键 |

> 快捷键在 macOS 上写作 `⇧⌘D` / `⇧⌘X`（思源内部统一用 macOS 字形存键位，
> 在 Windows 上由思源自己换算成 `Ctrl + Shift + …`）。
>
> **为什么不用空格**：这两条默认键位一开始是 `Ctrl + Alt + D` / `Ctrl + Alt + V`，
> 中间试过 `Ctrl + 空格` / `Alt + 空格`，结果**真机上按了没反应** ——
> `Ctrl + 空格` 是中文输入法「中/英文模式切换」的默认键（输入法在应用之前就把它吃掉，
> 而且**只在输入法处于活动状态时才吞**，所以表现成「有时灵、有时不灵」），
> `Alt + 空格` 是 Windows 的窗口系统菜单。两者都是**静默拦截**，没有任何报错。
>
> **为什么「并排打开」是 X 而不是 S 或 B**：S 看着更顺（Side / 并排），我们试过
> `Ctrl + Shift + S`，结果**在编辑器里按不动**。原因是 `Ctrl + S` 那一族归思源的
> 编辑器（Protyle）自己管，它会在事件上浮的路上把它拦下 ——
> 而思源的全局快捷键匹配器在更外层，**根本收不到这个按键**。
> 更麻烦的是它**只在光标位于编辑器内时失效**（焦点在文档树等别处时又能用），
> 又是一个「有时灵、有时不灵」。
>
> 接着换成 `Ctrl + Shift + B`（Beside，并排），栽在**更隐蔽的一层**：
> 按键确实送到了最外层，但思源内置的「上方插入块」（`editor.general.insertBefore`）
> 出厂就默认绑在这个键上，它在**更早的一步**把事件标记成「已处理」。
> 于是插件命令一次都没轮到 —— 画面毫无反应，正文里却**悄悄多出一个空段落**。
> 「送到没送到」和「轮不轮得到」是两道不同的关，只看前者会误判成「空闲」。
>
> 现在这组不含空格、也不含单独的 `Ctrl + Shift`（那个是切换输入语言），
> `Ctrl + Shift + X` 实测在思源侧未被任何命令或插件占用，按键也能正常送达。
>
> ⚠️ 思源内部把「空格」存成一个**字面空格字符**（不是 `Space` 这个词），
> 所以「查看快捷键」里会把空格渲染成可见的「空格」二字 ——
> 直接拼出来会变成 `Ctrl + ` 这样一个**看不见的尾随空格**，用户读不出该按什么。
> 速查里仍有这类行（折叠 / 展开、演示推进）。
>
> 思源会过滤插件声明的热键：**带 `⌃` / `⌥` / `⌘` 前缀的放行**，只有不带修饰键的
> 裸单字符才会被清空，所以这两条能正常绑上。
...
undo 栈）、`F5` / `F11` / `F12` 一律放行。

**`⌥⌘` 与 `⇧⌘` 两族也一律放行** —— 那是思源与其它插件共用的命名空间
（本工作空间里 flowmind、魔法排版、quick-notes、webview 等插件都在用），
导图必须整族让开。这条不是礼貌，是**数据安全**：导图自己绑了 `Ctrl + A / C / V / X / D / F`，
判据漏掉哪个修饰键，对应的那一族就会变成一次误操作 —— 其中 `⌥⌘X` / `⇧⌘X`
会照着「剪掉选中节点的子树」去改内核，等于**静默删掉你的笔记**。

> 插件自己的两条全局快捷键 `⇧⌘D` / `⇧⌘X` 也落在这一族里。
> 它们只在导图**没有**接管键盘时才走思源那条全局路径，两者不会互相吃掉；
> 导图有焦点时按 `⇧⌘D` 也不会被自己的 `Ctrl + D`（快速复制）截走。

如果不希望导图接管键盘，可在设置里关掉「导图内快捷键」。
...
设置面板是**侧边选项卡**式的，按用途分成五组，不用在一长条列表里找：

| 分区 | 装什么 | 条目 |
| --- | --- | --- |
| **外观** | 导图长什么样 | 主题、默认布局、连线样式、分支配色、自定义一级分支配色、显示层级编号、紧凑模式、布局动效 |
| **画布** | 画布多大、怎么自动取景 | 画布高度、画布高度（px）、自动适应画布、逻辑图自动分列、小地图、小地图始终显示 |
| **交互** | 鼠标和键盘的行为 | 双击编辑节点、拖拽调整节点、悬停预览折叠节点、导图内快捷键、`Ctrl + 滚轮缩放`、滚轮平移导图、视图偏好跟文档走、折叠状态与大纲同步 |
| **性能** | 大文档下的取舍 | 懒渲染、紧凑模式阈值、渲染上限 |
| **帮助** | 排查与迁移 | 查看快捷键、复制诊断信息、迁移【自定义块样式】标记 |

各分区里值得一提的：

- **滚轮行为** —— `Ctrl + 滚轮缩放` / `滚轮平移导图`，两者都关时滚轮交给页面
- **导图内快捷键** 关掉后方向键 / `Tab` 交还思源
- **「查看快捷键」** —— 弹出一张**分组表格对话框**（「键位 | 作用」两列，12 组 37 行），
  而不是一条十几秒就消失的通知。键位按平台换算：Windows 显示 `Ctrl + Shift + D` / `Ctrl + Shift + X`、
  macOS 显示 `⇧⌘D` / `⇧⌘X`；空格渲染成可见的「空格」二字（否则是看不见的尾随空白）；
  没有键位的行（点复选框、右键菜单这类）显式写「菜单操作」
- **「复制诊断信息」** —— 把版本、内核版本、配置快照、各视图状态与最近的插件警告
...
npm run ux:all     # 真机验收全家桶（19 支：交互 / 编辑 / 重开 / 折叠 / 过滤 / 标记 / 命令 / 设置项…）
npm run ux:settings   # 只跑设置项的**效果**（关 / 开对照，43 条断言）
npm run ux:settingsui # 只跑设置面板的**外壳**：侧边分区 + 快捷键速查（30 条断言）
npm run audit:coverage # 静态覆盖审计：从源码枚举全部 UI 入口 × tests/ 命中，产出缺口清单
```

> `ux:settings` 和 `ux:settingsui` 是刻意分开的两支：前者测「改一个开关，画布上有没有变化」，
> 靠「关 / 开 两次测量结果不同」自证；后者测「分组怎么排、点选项卡切不切、快捷键怎么显示」。
> 两者的失败原因完全不同，混在一起会让「面板白屏」和「开关没接线」看起来是同一类问题。
...
`addToggle/addSelect/addNumber/addText/addButton/addItem({title})`、以及那份对用户公开的
快捷键清单 —— 快捷键的「承诺」是从 `src/core/shortcuts.ts` 的数据表里读的，不是写死一份），
再逐条对照 `tests/` 里有没有出现过，输出 `✓ / △ / ✗` 和一份零命中清单。

> ⚠️ 这个脚本**自己会静默漏项**。本仓库开着 `core.autocrlf=true`，工作区是 CRLF，
> 而它原先用的解析正则都以 `\n` 收尾 —— 于是「解析 `onKeyDown` 里声明了哪些键」
> 那一节长期返回 **0 个键**，整节全是噪音。现在统一在读文件入口
> `.replace(/\r\n/g, "\n")` 归一化行尾。
> 教训：**用正则解析自己源码的脚本，第一件事就是归一化行尾**，
> 而不是逐个正则去兼容 `\r?\n`（漏一个就静默漏一项）。
...
```
src/
├── index.ts              插件入口：生命周期、块菜单（三种模式）、命令、设置面板、全屏视图
├── types.ts              公共类型与默认配置
├── core/
...
│   ├── shortcuts.ts      快捷键速查的**数据源**（12 组 37 行：键位 + i18n 键名）
│   └── settings-tabs.ts  把思源的扁平设置列表重排成侧边选项卡（含契约校验与回退）
├── utils/api.ts          思源内核 API 薄封装
├── utils/hotkey.ts       思源键位表示法 → 当前平台的可读写法
├── styles/index.css      全部样式（只挂在 .mm-root 之下；.mm-set / .mm-sc 例外，见文件头）
└── i18n/*.json           多语言（键必须**扁平**，见下）
```

> `core/shortcuts.ts` 是**单一数据源**：`addCommand` 的默认键位、菜单里的提示、
> 以及「查看快捷键」表格都读它。别在别处再手写一份键位 —— 改了绑定忘改说明，
> 用户按文档去按、按不动。
>
> `core/settings-tabs.ts` 为什么存在：思源的插件版 `Setting` **没有分组、没有选项卡、
> 也不暴露 DOM**（只有 `addItem` / `open`），所以分区只能自己实现 ——
> 在 `open()` 之后把 `.b3-dialog__content` 的子元素**搬**进自建的
> `.mm-set__nav` / `.mm-set__pane` 骨架里。它的核心约束是
> **契约校验先全查完再动 DOM，任何不符就一个节点都不动、退回思源的扁平列表**
> （宁可丑，不能丢设置项）；对应的硬断言是「`content` 直接子项里 `.config-item` 数 == 0」
> 与「各 pane 条目数之和 == 面板条目总数」，见 `tests/kernel/diag-settings-tabs.mjs`。
...
1. 跑 `npm run check` —— 8 道静态校验 + 317 条单元断言 + 96 条产物冒烟断言
...
> **思源 3.8.x** 上做的（3.8.4 起，后升级到 **3.8.5** 复跑过全量真机套件；
> 源码里的实测注释标的就是这两个版本）。

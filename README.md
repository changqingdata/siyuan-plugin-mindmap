...
| 快捷键 | `Ctrl + Shift + D` 切换导图 / 大纲，`Ctrl + Shift + B` 并排打开 | 可在思源「设置 → 快捷键」里改键 |

> 快捷键在 macOS 上写作 `⇧⌘D` / `⇧⌘B`（思源内部统一用 macOS 字形存键位，
...
>
> **为什么「并排打开」是 B 而不是 S**：S 看着更顺（Side / 并排），我们试过
...
> 又是一个「有时灵、有时不灵」。`B`（Beside，并排）实测没有这个问题。
>
> 现在这组不含空格、也不含单独的 `Ctrl + Shift`（那个是切换输入语言），
> 实测在思源侧未被任何命令或插件占用，按键也能正常送达。
>
...
- **「查看快捷键」** —— 弹出一张**分组表格对话框**（「键位 | 作用」两列，12 组 37 行），
  而不是一条十几秒就消失的通知。键位按平台换算：Windows 显示 `Ctrl + Shift + D` / `Ctrl + Shift + B`、
  macOS 显示 `⇧⌘D` / `⇧⌘B`；空格渲染成可见的「空格」二字（否则是看不见的尾随空白）；

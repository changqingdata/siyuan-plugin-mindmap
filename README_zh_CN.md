...
- **搜索与多选** —— `Ctrl + F` 定位节点，`Shift` 框选或 `Ctrl + A` 批量选中后统一操作
...
undo 栈）、`F5` / `F11` / `F12` 一律放行。如果不希望导图接管键盘，可在设置里关掉
「导图内快捷键」。
...
- 是否显示层级编号、是否分支配色、是否使用紧凑间距
- 滚轮行为（缩放 / 平移 / 交给页面滚动）
- 是否记忆折叠状态、是否自动适应画布
- 是否允许**双击编辑节点**、是否允许**拖拽调整节点**
- 是否启用**懒渲染**、紧凑模式阈值、渲染上限
- 是否启用**导图内快捷键**（关掉后导图不再接管键盘，方向键 / Tab 交还思源）
- 是否启用**布局动效**（增删节点时的位移过渡）
- 是否显示**小地图**（节点数少于 30 个时自动隐藏）
- **视图偏好跟文档走** —— 在某篇文档里改过布局 / 主题 / 连线 / 缩放，下次打开这篇文档还是它。
  只记**显式改过**的项，没改过的继续跟随上面的全局默认（否则改了全局设置所有文档都不跟着变）
- **悬停预览折叠节点** —— 关掉后悬停不再浮出子节点预览卡片
- **自定义一级分支配色** —— 逗号分隔的色值列表，留空则用主题自带的配色
- 「查看快捷键」按钮 —— 弹出完整的按键速查表
...
- 标记写在块属性 `custom-mindmap` 上（值为布局名），折叠状态写在 `custom-mindmap-fold` 上，可随笔记同步到其他设备。
...
│   ├── actions.ts        写回内核的结构操作：改名 / 增删 / 升降级 / 移动
